...
// https://sites.google.com/site/farshidfarhat/boy-names
const firstNameMale = [
...
// https://sites.google.com/site/farshidfarhat/girl-names
...
// http://fa.wikipedia.org/wiki/%D8%B1%D8%AF%D9%87:%D9%86%D8%A7%D9%85%E2%80%8C%D9%87%D8%A7%DB%8C_%D8%AE%D8%A7%D9%86%D9%88%D8%A7%D8%AF%DA%AF%DB%8C_%D8%B2%D8%A8%D8%A7%D9%86%E2%80%8C%D9%87%D8%A7%DB%8C_%D8%A7%DB%8C%D8%B1%D8%A7%D9%86%DB%8C
const lastName = [
...
const titleMale = ['آقای', 'استاد', 'دکتر', 'مهندس'];
...
export const generateNationalCode = () => {
    const area = randomNumber(0,999).toString().padStart(3, '0');
    const core = randomNumber(0,999999).toString().padStart(6, '0');
    const control = createControlCode(area, core);

    return  area + core + control;
}
...
const createControlCode = (area:string, core:string) => {
    const subNationalCodeString = area + core;

    let sum = 0;
    let count = 0;

    for (let i = 10; i > 1; --i) {
        sum += Number(subNationalCodeString[count]) * i;
        ++count;
    }
